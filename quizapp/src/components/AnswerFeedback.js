...
export function AnswerFeedback(props) {
    ...
    useEffect(() => {
        setCorrectCount(props.data.passedQuestions.filter((q, id) => Object.keys(q.answer_choices)
        .some(a => a === q.answer && props.data.quizResults.indexOf(a + '_' + id) !== -1)).length);
    }, []);

    return (
        <div className='p-3 col-6'>
            <h1 className="mb-5">Test results:</h1>
            {correctCount + '/' + props.data.passedQuestions.length}
